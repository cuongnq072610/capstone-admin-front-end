...
import { LOAD_ASK, LOAD_ASK_FAILURE, LOAD_ASK_SUCCESS, SEARCH_ASK_SUCCESS, SEARCH_ASK_FAILURE, SEARCH_ASK } from './constants';
import { fetchAsks } from './api';
import { API_ENDPOINT, GET_ALL_ASK, SEARCH_ASK_API } from '../../constants/apis';

function* loadAllAsks() {
  const user = JSON.parse(localStorage.getItem("user"));
  try {
    let response = yield call(fetchAsks, `${API_ENDPOINT}${GET_ALL_ASK}/${user.profile}`);
    if (response.data) {
